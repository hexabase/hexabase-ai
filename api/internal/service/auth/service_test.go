...
func (m *mockTokenDomainService) RefreshToken(ctx context.Context, session *auth.Session, user *auth.User) (*auth.Claims, error) {
...
func (m *mockTokenDomainService) ValidateRefreshEligibility(session *auth.Session) error {
	args := m.Called(session)
	return args.Error(0)
}

func (m *mockTokenDomainService) CreateSession(userID, refreshToken, deviceID, clientIP, userAgent string) (*auth.Session, error) {
...
func (m *mockTokenDomainService) ValidateTokenClaims(claims *auth.Claims) error {
	args := m.Called(claims)
	return args.Error(0)
}

func (m *mockTokenDomainService) ShouldRefreshToken(claims *auth.Claims) bool {
	args := m.Called(claims)
	return args.Bool(0)
}
