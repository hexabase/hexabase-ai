...
func NewNodeHandler(service node.Service, logger *zap.Logger) *NodeHandler {
	return &NodeHandler{
		service: service,
		logger:  logger,
	}
...
func (h *NodeHandler) GetAvailablePlans(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get available plans", zap.Error(err))
...
func (h *NodeHandler) GetPlanDetails(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get plan details", zap.Error(err), zap.String("plan_id", planID))
...
func (h *NodeHandler) ProvisionDedicatedNode(c *gin.Context) {
	...
	h.logger.Info("dedicated node provisioned",
		zap.String("node_id", dedicatedNode.ID),
		zap.String("workspace_id", workspaceID),
		zap.String("user_id", userID),
		zap.String("node_name", req.NodeName),
		zap.String("node_type", req.NodeType))
...
func (h *NodeHandler) ListNodes(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to list nodes", zap.Error(err), zap.String("workspace_id", workspaceID))
...
func (h *NodeHandler) GetNodeStatus(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get node status", zap.Error(err), zap.String("node_id", nodeID))
		c.JSON(http.StatusNotFound, gin.H{"error": "node not found"})
		return
	}
...
func (h *NodeHandler) GetNodeMetrics(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get node metrics", zap.Error(err), zap.String("node_id", nodeID))
...
func (h *NodeHandler) GetNodeEvents(c *gin.Context) {
	...
	if err != nil {
		h.logger.Error("failed to get node events", zap.Error(err), zap.String("node_id", nodeID))
